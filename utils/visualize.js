...
function showClassAccuracy(labels, preds, class_names) {
  tfvis.metrics
    ...
    .then((classAccuracy) => {
      ...
      tfvis.show.perClassAccuracy(container, classAccuracy, class_names)
    })
}
...
function showConfusionMatrix(labels, preds, class_names) {
  tfvis.metrics
    ...
    .then((confMatrix) => {
      ...
      tfvis.render.confusionMatrix(container, confMatrix, class_names)
    })
}
...
function showActivations(data, model) {
  ...
  renderImageTable(conv1ActivationSurface_drawArea, filters, filterActivations)
  renderImageTable(
